 ...
 * always the same `npx -y browser-memory`:
...
export interface HostResult {
  ...
  status: "added" | "already" | "removed" | "absent" | "unavailable" | "error";
...
function claudeInstall(): HostResult {
  if (!commandExists("claude"))
    return {
      ...
      status: "unavailable",
      message:
        "Claude Code CLI not on PATH — run: claude mcp add --scope user browser-memory -- npx -y browser-memory",
    };
